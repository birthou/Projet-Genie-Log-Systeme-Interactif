...
namespace Projet_CasseBrique
{
    public class Balle : Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        {
            spriteBatch.Begin();
            spriteBatch.Draw(uneballe.Texture, uneballe.Position, Color.DarkRed);
            spriteBatch.End();
            base.Draw(gameTime);
        }
        ...
        public void BougeBalle()
        {
            ...
            if (uneballe.Position.Y >= maxY)
            {
                ...
                this.Nbreballes--;
                raquette.TailleX -= 45;
                if (!CasseBrique.Agagne)
                    score -= 25;
                estDemarre = false;
                if (Nbreballes == 2)
                    raquette.Uneraquette.Texture = Game.Content.Load<Texture2D>("Player");
                if (Nbreballes == 1)
                    raquette.Uneraquette.Texture = Game.Content.Load<Texture2D>("Player");
        ...
        private void gestionCollision()
        {
            ...
            v = uneballe.Vitesse;
            if (ElementText2D.testCollision(this, this.raquette.Bbox))
            {
                ...
                posRel = ElementText2D.getRelativePosition(infosBalle, infosRaquette);

                if (posRel[0] == ElementText2D.CROISEMENT)
                {
                    v.Y *= -1;
                ...
                if (posRel[1] == ElementText2D.CROISEMENT)
                {
                    v.X *= -1;
        ...
        private void gestionCollisionBrique()
        {
            ...
            while (x < NBLIGNES && !collision)
            {
                ...
                while (y < NBCOLONNES && !collision)
                {
                    ...
                    if (!unebrique.Marque && ElementText2D.testCollision(this, bbox_brique))
                    {
                        ...
                        posRel = ElementText2D.getRelativePosition(infosBalle, infosBrique);

                        if (posRel[0] == ElementText2D.CROISEMENT && (posRel[1] == ElementText2D.AU_DESSUS || posRel[1] == ElementText2D.EN_DESSOUS))
                        {
                            v.Y *= -1;
                        ...
                        // Si les 2 objets se croisent sur l'axe des Y
                        else if (posRel[1] == ElementText2D.CROISEMENT)
                        {
